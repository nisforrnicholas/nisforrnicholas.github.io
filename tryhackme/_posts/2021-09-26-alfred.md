...
Now let's migrate to the process using the following command:
...
We now spawn a privileged shell using the `shell` command. 

We can then read the **root.txt** file located in C:\Windows\System32\config using the `TYPE` command:
