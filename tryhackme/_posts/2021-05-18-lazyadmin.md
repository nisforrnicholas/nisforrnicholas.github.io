...
<div class="notice">
  ...
  <p>While rewriting this writeup, I noticed that the old me had completely misunderstood the function of the copy.sh script. If we look carefully, we can see that it's actually a reverse shell script :sweat_smile:

  Hence, I could have just reused copy.sh and replaced the IP address in it with my own. Then, when running perl with sudo on the backup.pl file, copy.sh will then be called, causing a reverse shell with root privileges to be sent back to me. 
  
  There was no need to replace the script with /bin/sh, as you'll see later.

  Oh wells, we live and we learn!</p>
</div>
