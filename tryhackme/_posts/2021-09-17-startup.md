...
Gobuster manages to find a **/files** directory! Visiting this directory, we can see all the files that was on the FTP server earlier on:
...
We will be using the PHP reverse shell script created by [pentestmonkey](https://github.com/pentestmonkey/php-reverse-shell). To upload the script onto the FTP server, we use the `put` command. I uploaded the reverse shell script onto the 'ftp' directory in the FTP server. Upon doing so, we can see the uploaded file on the web server:
...
And then open the file from within the web browser. The reverse shell will then be opened:
...
planner.sh is globally readable, so let's find out what it does:

![screenshot24](../assets/images/startup/screenshot24.png)

It looks like it echoes the contents of the $LIST env variable into startup_list.txt, before running the /etc/print.sh script. 

Let's now take a look at /etc/print.sh:
