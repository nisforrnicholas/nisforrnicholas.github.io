...
We also see that the FTP server allows for **anonymous login**. This is a common misconfiguration on FTP servers, allowing us to access the files within the server without a password.
...
Let's focus on the first few lines of hexadecimal characters that make up the file. This is because the magic bytes will always be placed at the very beginning of the file. Looking at the following [wikipedia page](https://en.wikipedia.org/wiki/List_of_file_signatures#:~:text=This%20is%20a%20list%20of,its%20contents%20will%20be%20unintelligible):
...
It would also seem that mailto has not been properly configured as it points to '#', instead of an email address. Looking at the source code of the page, we can confirm this as the developer has left a comment:
...
Next,let's run a `gobuster` scan to try and find any hidden directories on this web server:
...
Gobuster manages to find a **/files** directory! 

Visiting this directory, we can see all the files that was on the FTP server earlier on:
...
We will be using the PHP reverse shell script created by [pentestmonkey](https://github.com/pentestmonkey/php-reverse-shell). 

To upload the reverse shell script onto the FTP server, we use the `put` command. I uploaded the script to the 'ftp' directory on the FTP server. Upon doing so, we can see the uploaded script on the web server:
...
Also within the root directory, we can also see an interesting directory called **incidents**. This directory contains a pcap file called **suspicious.pcapng**:
...
'Documents' contains 3 text files: **concern.txt**, **list.txt** and **note.txt**. These files did not seem to contain anything that we can use to escalate our privileges.
...
Also, planner.sh is globally readable, so let's find out what it does:

![screenshot24](../assets/images/startup/screenshot24.png)

It looks like it echoes the contents of the $LIST env variable into startup_list.txt, before running a script called **/etc/print.sh**. 

Let's take a look at /etc/print.sh:

![screenshot25](../assets/images/startup/screenshot25.png)

All it does is print "Done!" into the terminal. 

With that said, print.sh actually belongs to lennie! This means that we are able to write over it and have it execute anything we want, such as opening a reverse shell. We'll be using the bash reverse shell from [PayloadsAllTheThings](https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/Methodology%20and%20Resources/Reverse%20Shell%20Cheatsheet.md):
...
At this point, I had an inkling that planner.sh was most probably being run as a cronjob by root. To verify this, we can use a nifty program called [pspy](https://github.com/DominicBreuker/pspy/blob/master/README.md), which allows us to snoop on processes without the need for root permissions.

After uploading the pspy executable onto the SSH server, we then execute it:

![screenshot26](../assets/images/startup/screenshot26.png)

As we can see, planner.sh was automatically run with the UID=0 (root). This confirms that it is indeed a cronjob belonging to root!
